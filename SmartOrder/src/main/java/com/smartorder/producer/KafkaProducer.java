...
public class KafkaProducer {
    ...
    private void sendWithTopicCheck(Object event, String orderId, String eventType) {
        try {
            ...
            kafkaTemplate.send(TOPIC_NAME, event).get();
            ...
            if (ex.getMessage() != null && ex.getMessage().contains("UNKNOWN_TOPIC_OR_PARTITION")) {
                ...
                try {
                    ...
                    System.err.println("[KafkaProducer] Retry failed: " + retryEx.getMessage());
                }
            } else {
                System.err.println("[KafkaProducer] Failed to send event for order " + orderId + ": " + ex.getMessage());
            }
        }
    }
    ...
    private void createTopicIfMissing(String topicName) {
        ...
        try (AdminClient admin = AdminClient.create(configs)) {
            ...
            if (e.getCause() instanceof TopicExistsException) {
                System.out.println("ℹ[KafkaProducer] Topic already exists: " + topicName);
            } else {
                System.err.println("[KafkaProducer] Topic creation failed: " + e.getMessage());
            }
        } catch (Exception e) {
            System.err.println("[KafkaProducer] Error while creating topic: " + e.getMessage());
        }
    }
